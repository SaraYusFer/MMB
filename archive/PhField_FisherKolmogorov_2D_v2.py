...
# initialize nutrient field c (well-perfused outside tumor)
c = np.ones((Lx, Ly)) * c_inf
...
for t in tqdm(np.arange(0, tmax, dt)):
    ...
    for k in range(Nrho):
        # Monod nutrient dependence
        growth = r[k] * (c / (c + Kc)) * u[:, :, k] * (1.0 - U_tot / K_U)
        death = -d[k] * u[:, :, k]
    ...
    if (round(t/dt)%1==0):
        ...
        ax.set_title("Phenotype Overlay (mixed color)")
        buf = io.BytesIO()
        fig.savefig(buf, format='png')
        buf.seek(0)
...
imageio.mimsave(f'results/v2/simulation_{timestamp}.gif', frames, fps=10)
imageio.mimsave(f'results/v2/simulation_rho_{timestamp}.gif', rho_overlay_frames, fps=10)
