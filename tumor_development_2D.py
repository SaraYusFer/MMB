"""
Modeling tumor development under varying environmental and internal conditions.
...
epsilon2 = 4.0               # controls interface width (tumor diffuse edge)
M = 1.0                      # motility
alpha_eps = 0.3              # controls how strongly the phenotype field modifies the phase-field interfacial width.
D_rho = np.array([0.1, 0.65])  # diffusion coefficient per phenotype bin. We only use high and low aggresivity phenotypes

# nutrient parameters
D_c = 0.05        # nutrient diffusion
gamma_c = 2.0     # consumption coefficient
c_inf = 1.0       # background nutrient level
lambda_c = 0.01   # relaxation to background
s = 0.0575        # nutrient supply (g/L)

# phenotype-specific rates
r = np.array([0.2, 0.8])   # proliferation rates: low, high
...
cx, cy = Lx//2, Ly//2
radius = 12.0      
edge_width = 5.0   # How gradual the edge is (larger -> smoother)
...
# Nutrient field
# initialize nutrient field : gradient 
c = c_inf * (1.5 - 2.5 * (np.add.outer(np.arange(Lx), np.arange(Ly)) / (Lx + Ly - 2)))
# Reduce nutrients inside initial tumor core to mimic consumption
c[phi > 0.5] = 0.8 * c_inf
...
for t in tqdm(np.arange(0, tmax, dt)):
    ...
    # normalize U_tot to [0,1] by dividing by normalizing constant
    m = np.clip(U_tot / 1.0, 0.0, 1.0)
    ...
    Kc = 0.05   # half-saturation constant for nutrient
    cons = c / (c + Kc)
    ...
    c_lap = (c[sright, :] + c[sleft, :] + c[:, sup] + c[:, sdown] - 4.0 * c) # discrete laplacian for c
    uptake = gamma_c * (q[0] * u[:, :, 0] + q[1] * u[:, :, 1]) * phi 
    supply = s * (1-phi) # only supply nutrients outside of tumor
    decay = lambda_c * (c - c_inf)
    dc_dt = D_c * c_lap + supply - uptake - decay
    ...
    if (round(t/dt)%1==0):
        ...
        # Tumor pixels only
        tumor_indices = np.argwhere(phi > 0.01)
        for i, j in tumor_indices:
            ...
            low_frac_vis = np.sqrt(low_frac)
            high_frac_vis = 1 - low_frac_vis
            rgb_img[i, j, :] = low_frac_vis * np.array([1, 1, 0]) + high_frac_vis * np.array([1, 0, 0])
...
imageio.mimsave(f'results/simulation_{timestamp}.gif', frames, fps=10)
imageio.mimsave(f'results/simulation_rho_{timestamp}.gif', rho_overlay_frames, fps=10)
imageio.mimsave(f'results/simulation_nutrients_{timestamp}.gif', nutrient_frames, fps=10)
